...
const GetRandomCard = ({icon, buttonText, copyBtn, bgColor, textColor, handleClick, data}) => {
  ...
  return (
    <div className={`rounded-md shadow-xl p-8 md:px-10 xl:px-14 relative bg-white ${textColor}`}>
         <Link to="/">
            <button className={`${bgColor} text-white p-1 rounded-full text-xs md:text-base absolute top-3 left-2 md:left-3`}>
                <IoArrowBackOutline/>
            </button>
        </Link>
        {icon}
        <p className="mt-5 mb-6 xl:mt-6 xl:mb-7 text-sm md:text-base xl:text-lg tracking-wide md:tracking-normal text-slate-800 text-center">
            {data}
        </p>
        <div className="flex items-center justify-around md:justify-evenly text-xs md:text-sm xl:text-base">
